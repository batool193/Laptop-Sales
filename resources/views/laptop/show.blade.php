                    ...
                    <div class="d-flex align-items-center justify-content-between flex-wrap">
                        <div>
                            @foreach ($laptop->attachements as $attachement)
                                <img src="{{ Storage::url($attachement->file_path) }}" alt="{{ $laptop->brand }}"
                                    width="150" class="img-thumbnail mr-2 mb-2">
                            @endforeach
                        </div>
<a href="{{ route('buy.form', ['laptop' => $laptop->id]) }}" class="btn btn-success btn-lg px-4 py-3">Buy</a>
            ...
            <div class="card border-warning shadow-sm">
                ...
                <div class="card-body">
                    @foreach ($laptop->offers as $offer)
                        <div class="mb-3">
                            <label class="d-block"><strong>Offer Description:</strong>
                                {{ $offer->offer_description }}</label>
                            <label class="d-block"><strong>Discount Percentage:</strong>
                                {{ $offer->discount_percentage }}%</label>
